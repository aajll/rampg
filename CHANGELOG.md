...
- `rampg_set_enabled()` and `rampg_is_enabled()` for enable/disable. A disabled ramp holds its output: `rampg_update` leaves the value unchanged and `rampg_get_rate` reads zero, while `rampg_at_target` and `rampg_get_state` keep reporting the underlying movement. The ramp is enabled by default after `rampg_init`, and re-enabling resumes the move from the current value. The new `enabled` field is appended at the end of `rampg_t`, inside existing tail padding, so `sizeof(rampg_t)` and all existing field offsets are unchanged.
