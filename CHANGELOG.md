...
- `rampg_set_accel()` and the `RAMPG_DEFAULT_ACCEL` option, setting the acceleration limit in units per second squared. Ignored by the linear profile.
...
### Changed
- `rampg_t` gains `accel` and `vel` and no longer carries per-move planner state. `sizeof(rampg_t)` is 40 bytes on x86_64. Only `vel` is internal; every other field is caller-facing configuration.
